...
namespace ations
{
  public class Stats : DependencyObject, INotifyPropertyChanged
  {
    ...
    public bool IsWar { get { return WarCard != null; } }
    public Card WarCard { get { return warCard; } set { warCard = value; NotifyPropertyChanged(); NotifyPropertyChanged("IsWar"); } }
    Card warCard;
    public Point WarPosition { get { return warPosition; } set { warPosition = value; NotifyPropertyChanged(); } }
    Point warPosition;
    public int WarLevel { get; set; }

    public bool IsEndOfAge { get { return Round % 2 == 1; } }

    Game game;

    public void Clear()
    {
      WarCard = null; UpdateWarPosition(); Architects = Turmoils = 0; EventCard = null;
    }
    ...
    public void UpdateWarPosition(Player pl = null, Card card = null)
    {
      ...
      else
      {
        ...
        Debug.Assert(card != null, "UpdateWarPosition without card!");
        WarCard = card;
      }
    }
    ...
    public void PickEventCard(Card card=null)
    {
      if (card == null)
      {
    ...
    public List<Card> GetEventCards(int n)
    {
      ...
      for (int i = 0; i < n; i++) { result.Add(Card.MakeEventCard(EventCardDeck.First()));        EventCardDeck.RemoveAt(0); }
      return result;
    }
    public List<Card> PeekEventCards(int n)
    {
      if (EventCardDeck == null || EventCardDeck.Count() < 0) UpdateEventDeck(Age); // just in case run out of event cards (which doesnt normally happen)
      var result = new List<Card>();
      for (int i = 0; i < n; i++) { result.Add(Card.MakeEventCard(EventCardDeck.First())); }
      return result;
    }
