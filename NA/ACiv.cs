...
namespace ations
{
  public class ACiv
  {
    ...
    public List<AField> Fields { get; set; }
    ...
    public ACiv(string civ)
    {
      ...
      for (int i = 0; i < xfields.Length; i++)
      {
        ...
        Fields.Add(new AField { X = xfield, Type = type, Index = i, TypesAllowed = types, Margin = margin });
      }
    }

    public ObservableCollection<ACard> GetCards()//index from 0 to 13 for 2 rows of 7 cards each!
    {
      var result = new ObservableCollection<ACard>();
      foreach (var f in Fields) result.Add(ACard.MakeCivCard(f, Name));
      return result;
    }

    public AResDict GetResources()
    {
      var result = new AResDict();
    ...
    public ObservableCollection<AWorker> GetExtraWorkers()
    {
      var result = new ObservableCollection<AWorker>();
      ...
      foreach (var res in resarr)
      {
        ...
        result.Add(new AWorker(costres, res.aint("n"), margin, false)); //testing
      }
      return result;
    }

    public void Add(ACard card, AField field) { field.Card = card; }
